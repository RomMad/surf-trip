const csrfFieldSelector = 'input[data-controller="csrf-protection"], input[name="_csrf_token"]';
...
document.addEventListener('turbo:submit-start', (event: Event) => {
    ...
    const headers = generateCsrfHeaders(customEvent.detail.formSubmission.formElement);
...
document.addEventListener('turbo:submit-end', function (event: Event) {
    const customEvent = event as CustomEvent<{ formSubmission: { formElement: HTMLFormElement } }>;

    removeCsrfToken(customEvent.detail.formSubmission.formElement);
});

export function generateCsrfToken (formElement: HTMLFormElement) {
    const csrfField = formElement.querySelector<HTMLInputElement>(csrfFieldSelector);
    ...
    if (!csrfCookie && nameCheck.test(csrfToken)) {
        ...
        csrfField.defaultValue = csrfToken = btoa(String.fromCharCode.apply(null, Array.from(bytes)));
    }
...
export function generateCsrfHeaders (formElement: HTMLFormElement): Record<string, string> {
    ...
    if (tokenCheck.test(csrfField.value) && csrfCookie !== null && nameCheck.test(csrfCookie)) {
